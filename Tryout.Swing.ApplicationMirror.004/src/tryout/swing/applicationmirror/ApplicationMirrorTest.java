 ...
 *
 * In der Version .003 des Tryouts gilt:
 * 20180606: Schicker gemacht
 ...
 *      
 *  b) Jeder getippte Buchstabe erscheint sofort auf dem Server, ohne dass extra der Button geclickt werden muss.
 *  c) Das Clicken des Buttons setzt den Text leer.
 *  
 *  TODO GOON 20180606:
 *  Lag der Schwerpunkt in der Version .003 auf dem Versenden eines Objekts, 
 *  so sollte in .004 wieder auf den AWTEvents selbst mehr geachtet werden.
 *  Vielleicht Die Robot Klasse einsetzen, so dass man die Mauszeiger-Bewegung spiegelt.
 *  
 *
 */
...
//                             out.writeObject(evt);
	...
	//                }
                ...
                if(obj instanceof MyMessage){
	                ...
	                if(objMessage!=null){
	                	 ...
	                	 //      Aus der objComponent nun das JTextfield Objekt holen.
    ...
    public void launchUI(){
    	 ...
    	 //Diesen Code aus dem Konstruktor hierher verschoben. 
    	 //Idee: Nach dem Start und dem Socket-Aufbau steht fest, ob die Applikation als Server oder Client agiert.
    	 //      Dann kann man entsprechend, z.B. ein Label einblenden.
    	 //ABER: Das muss unbedingt VOR dem socket.accept() passieren. Sonst wartet der Server OHNE UI auf die Eingabe 
    	 //      und da noch kein UI vorhanden ist, kann er nix spiegeln. (component xyz nicht gefunden).
    	
         frame.getContentPane().setLayout(new FlowLayout());
         frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);  //Merke: Damit werden die Anwendungen auch geschlossen, nicht nur das UI.
         
         if(this.isServer()==true){
        	 JLabel labelServer = new JLabel();
        	 labelServer.setText("SERVER: " + this.getServerUrl() + ":" + this.getServerSocketPort());
        	 frame.getContentPane().add(labelServer);
         }
         
         final JButton button = new JButton("Lösche Text. Clicke -> übertrage an Server den Click selbst.");
         button.setName("button");
         
         //Allerdings wird der Text nicht übertragen.
         JTextField tf = new JTextField();
         tf.setName("textfield01");
         if(this.isServer()==true){
        	 tf.setEnabled(true);
        	 ...
        	 tf.setEnabled(true);
        	 tf.setEditable(true);
        	 tf.setText("Text. Zum Übertragen clicken...");  
        	 
        	 MyDocumentListenerForTextfield myDocListener = new MyDocumentListenerForTextfield(this, tf);        	 
        	 tf.getDocument().addDocumentListener(myDocListener);
         }
